...
min_genotype_count=10 # Minimum number of times a genotype needs to apper to get a core microbiome for it
...
#asvs = readRDS("3_GxE/3a_asv_table.rarefied.filt_for_gxe.rds") # If want to be consistent with GxE analysis
metadata = asvs %>% 
  sample_data() %>%
  data.frame() %>%
...
collapsed_taxa = mclapply(ranks, function(myrank){
  tax_glom(asvs, taxrank = myrank) # Why do samples end up with < 1500?
}, mc.cores=ncores)
names(collapsed_taxa) = ranks

# Make sample depth key (needed for calculating % of reads since the
#    taxa glom above drops anything with missing info)
depthkey = sample_sums(asvs)
depthkey = data.frame(SampleID=names(depthkey), total_count=depthkey)
...
core_microbiome_preparse <- function(tax_level, mydata, collapsed_taxa){
  ...
  collapsed_counts = otu_table(collapsed) %>%
    ...
    left_join(depthkey, by="SampleID") %>%
    group_by(SampleID) %>%
    mutate(fraction=count / total_count, present = fraction >= min_fraction)%>% # True/false for if taxon present in a sample at at least min_fraction abundance
...
location_prevalence = lapply(ranks, function(myrank){
  prevalence = preparse[[myrank]] %>%
    ...
    summarize(prevalence = sum(present) / n(), .groups="drop") #TODO - Need to ignore NAs here? but where coming from?
  return(prevalence)
})
...
# Filter for genotypes that have enough counts to be core
genotype_counts = table(metadata$Corrected_pedigree)
enough_genos = genotype_counts[genotype_counts >= min_genotype_count]
genos_with_cores = names(enough_genos)

# Calculate prevalence
genotype_prevalence = lapply(ranks, function(myrank){
  prevalence = preparse[[myrank]] %>%
    filter(Corrected_pedigree %in% genos_with_cores) %>%
...
##############
# Output tables of results (including overall dataset, not just geno/location)
##############
...
# Cut above for easier parsing
overall_cores_output_pub = overall_cores_output %>%
  filter(prevalence >= min_prevalence) %>%
  arrange(taxon_name)
write.csv(overall_cores_output_pub, file="4_CoreMicrobiome/4c_prevalence_summary.overall_pub.csv", row.names=FALSE)


# ####################
# # Overall heatmap of all samples - DEPRECATED
# ####################
# 
